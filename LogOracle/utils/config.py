...
class Configurable(object):
    def __init__(self, args, config_file, extra_args=[]):
        config = ConfigParser()
        ...
        self._config = config
        output_dir = os.path.join('dataset/process', args.module)
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        config.write(open(os.path.join(output_dir, 'config.cfg'), 'w'))
    ...
    @property
    def trace_dim(self):
        return self._config.getint("Embedding", "trace_dim")
    
    @property
    def tree_width(self):
        return self._config.getint("Embedding", "tree_width")

    @property
    def trace_id_dim(self):
        return self._config.getint("Embedding", "trace_id_dim")

    @property
    def trace_seq_max_len(self):
        return self._config.getint("Embedding", "trace_seq_max_len")

    @property
    def max_log_len(self):
        return self._config.getint("Embedding", "max_log_len")
    ...
    @property
    def label_dims(self):
        return self._config.getint('Network', 'label_dims')
